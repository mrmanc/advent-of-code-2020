...
input.forEach(boardingPass => {
    let currentSeatID = seatID(boardingPass);
    seats[currentSeatID] = true;
    if(currentSeatID>highestSeatID) highestSeatID = currentSeatID; 
})
console.log(`Part 1: ${highestSeatID}`);
for(let currentSeatID:number = 0, seenSeat:boolean = false; currentSeatID <= highestSeatID; currentSeatID++) {
    if (seats[currentSeatID]) seenSeat = true;
    else if (seenSeat) console.log(`Part 2: ${currentSeatID}`);
}
